...
fn expand(data: Data, root: TokenStream, struct_on_set: TokenStream) -> (TokenStream, TokenStream) {
    ...
    let const_defs = fields.iter().map(|(_, _, event_ident, event_str)| {
        quote! {
            pub const #event_ident: &str = #event_str;
        }
    });
    ...
    let get_set_impls = fields.iter().map(|(attrs, ident, event_ident, _)| {
        ...
        let field_on_set = attrs.on_set.as_ref().map(|on_set| {
            quote! {
        ...
        quote! {
            ...
            fields.add_field_method_set(stringify!(#ident), |lua, this, val| {
                ...
                this.#ident = val;
                #field_on_set
                #struct_on_set(lua, Self::#event_ident, &this.#ident)
            })
        }
    });
